...
def convert_to_standard(filepath):
    ...
    try:
        ...
        # 1. CONVERSÃO JÁ CONCLUÍDA - SISTEMA AGORA USA DIÁLOGOS PADRÃO
        # Todos os imports já foram convertidos para biodesk_dialogs (padrão PyQt6)
        ...
        content = re.sub(r'BiodeskMessageBox\.information\s*\(', 'QMessageBox.information(
            ',
            content)
        content = re.sub(r'BiodeskMessageBox\.warning\s*\(',
            'QMessageBox.warning(
            ',
            content
        )
        content = re.sub(r'BiodeskMessageBox\.critical\s*\(',
            'QMessageBox.critical(
            ',
            content
        )
        content = re.sub(r'BiodeskMessageBox\.question\s*\(',
        ...
        content = re.sub(r'mostrar_informacao\s*\(',
        ...
        content = re.sub(r'mostrar_sucesso\s*\(', 'QMessageBox.information(
            ',
            content)
        content = re.sub(r'mostrar_aviso\s*\(',
            'QMessageBox.warning(
            ',
            content
        )
        content = re.sub(r'mostrar_erro\s*\(',
            'QMessageBox.critical(
            ',
            content
        )
        content = re.sub(r'mostrar_confirmacao\s*\(',
            'QMessageBox.question(
            ',
            content
        )
